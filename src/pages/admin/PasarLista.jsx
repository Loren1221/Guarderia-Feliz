...
const PasarLista = () => {
  ...
  // Cargar estudiantes y asistencia cuando cambia la fecha
  useEffect(() => {
    const fetchDatos = async () => {
      ...
      try {
        ...
        // Crear un mapa de asistencia para la fecha actual
        const asistenciaMap = {};
        estudiantesData.forEach((estudiante) => {
          const registro = asistenciaData.find((a) => a.id_estudiante === estudiante.id);
          asistenciaMap[estudiante.id] = registro ? registro.estado : "ausente";
        });
  ...
  const estudiantesPresentes = estudiantes.filter(
    (estudiante) => asistencia[estudiante.id] === "presente"
  );
  const estudiantesAusentes = estudiantes.filter(
    (estudiante) => asistencia[estudiante.id] === "ausente"
  );
