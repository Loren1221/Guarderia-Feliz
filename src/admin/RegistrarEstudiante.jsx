...
const RegistrarEstudiante = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "id_padre") {
      const padreSeleccionado = padres.find((padre) => padre.id === parseInt(value));
      setEstudiante({
        ...estudiante,
        id_padre: { id: padreSeleccionado.id, name: `${padreSeleccionado.name} ${padreSeleccionado.last_name}` },
      });
    } else if (name === "sexo") {
      const sexoSeleccionado = sexos.find((sexo) => sexo.id === parseInt(value));
      setEstudiante({
        ...estudiante,
        sexo: { id: sexoSeleccionado.id, name: sexoSeleccionado.name },
      });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !estudiante.nombre ||
      !estudiante.apellido ||
      !estudiante.sexo.name ||
      !estudiante.fecha_nacimiento ||
      !estudiante.id_padre.name ||
      !estudiante.direccion ||
      !estudiante.monto // Verificar si el monto es completado
    ) {
    ...
    try {
      ...
      const { error } = await supabase.from("Estudiantes").insert({
        ...
        sexo: estudiante.sexo.name, // Guardar el nombre del sexo
        fecha_nacimiento: estudiante.fecha_nacimiento,
        id_padre: estudiante.id_padre.name, // Guardar el nombre del padre
        direccion: estudiante.direccion,
        telefono_contacto: estudiante.telefono_contacto,
        alergias: estudiante.alergias,
        observaciones: estudiante.observaciones,
        monto: estudiante.monto, // Guardar el monto
      });
  ...
  const handleGenerateReceipt = () => {
    ...
    doc.text(`Sexo: ${estudiante.sexo.name}`, 20, 50);
    doc.text(`Nombre del Padre: ${estudiante.id_padre.name}`, 20, 60);
